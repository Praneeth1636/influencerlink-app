...
import { connectTikTok } from "@/server/services/platform-service";
import { logger } from "@/lib/logger";

export const runtime = "nodejs";
const log = logger.child({ module: "tiktok-oauth-callback" });
...
export async function GET(req: NextRequest) {
  ...
  if (!creator) return settled("no_creator", "Finish creator onboarding before connecting TikTok");

  try {
    const result = await connectTikTok(db, userRow, creator, { code });
    log.info({ creatorId: creator.id, username: result.username, followers: result.followers }, "tiktok connected");
    const res = settled("ok", `Connected @${result.username}`);
    res.cookies.delete(STATE_COOKIE);
    return res;
  } catch (err) {
    log.error({ err }, "tiktok connect failed");
