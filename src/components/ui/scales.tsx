...
export function Scales({ orientation = "vertical", className }: { orientation?: "vertical" | "horizontal"; className?: string }) {
  ...
  return (
    <div className={cn("absolute inset-0 overflow-hidden opacity-50", className)} aria-hidden="true">
      {Array.from({ length: 28 }, (_, index) => (
        <span
          className={cn("absolute bg-gradient-to-b from-transparent via-sky-500/30 to-transparent", vertical ? "h-full w-px" : "h-px w-full")}
          key={index}
